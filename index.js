...
function isEvenNumber(num) {
  let checkNumParity = num % 2 === 0 ? true : false // Перевірити, чи число num є парним
  return checkNumParity // Повернути результат перевірки (true або false) як результат функції
}
...
function checkDataType(value) {
  let dataType = typeof value // Отримуємо тип значення value за допомогою оператора typeof
  console.log(`Значення "${value}" має тип "${dataType}"`) // Виводимо у консоль повідомлення, яке містить значення value та його тип
...
function memoizedArea() {
  ...
  return function (side) {
    ...
    if (side === prevSide) {
      ...
      let result = side * side // Розраховуємо площу
